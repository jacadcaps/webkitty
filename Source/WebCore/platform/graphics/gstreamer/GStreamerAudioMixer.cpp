...
GRefPtr<GstPad> GStreamerAudioMixer::registerProducer(GstElement* interaudioSink, std::optional<int> forcedSampleRate, const String& deviceId, const GRefPtr<GstDevice>& device)
{
    ...
    GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(GST_BIN_CAST(mp.pipeline.get()), GST_DEBUG_GRAPH_SHOW_ALL, "audio-mixer-after-producer-registration");
    return mixerPad;
}
...
void GStreamerAudioMixer::unregisterProducer(const GRefPtr<GstPad>& mixerPad)
{
    ...
    if (!mp.mixer->numsinkpads) {
        ...
        GST_DEBUG_BIN_TO_DOT_FILE_WITH_TS(GST_BIN_CAST(mp.pipeline.get()), GST_DEBUG_GRAPH_SHOW_ALL, "audio-mixer-after-producer-unregistration");
    }
}
