...
void FEColorMatrixSoftwareApplier::applyPlatform(PixelBuffer& pixelBuffer) const
{
    ...
    switch (m_effect.type()) {
    case FECOLORMATRIX_TYPE_MATRIX:
       applyMatrixFast(m_effect.values(), pixelArray.data(), pixelBuffer.size().width(), pixelBuffer.size().height());
       return;
    case FECOLORMATRIX_TYPE_SATURATE:
    case FECOLORMATRIX_TYPE_HUEROTATE:
       applyHueSaturateFast(m_components, pixelArray.data(), pixelBuffer.size().width(), pixelBuffer.size().height());
       return;
       case FECOLORMATRIX_TYPE_LUMINANCETOALPHA:
       applyLuminanceFast(pixelArray.data(), pixelBuffer.size().width(), pixelBuffer.size().height());
       return;
    default:
        break;
    }
