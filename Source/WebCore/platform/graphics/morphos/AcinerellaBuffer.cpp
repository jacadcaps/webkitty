...
// dprintf("-- AcinerellaNetworkBufferInternal ORPHANED!!\n");
	...
	Ref<CurlRequest> createCurlRequest(ResourceRequest&request)
	{
		...
		if (context)
		{
			...
			String cookieHeaderField = storageSession.cookieRequestHeaderFieldValue(request.firstPartyForCookies(), SameSiteInfo::create(request), request.url(), std::nullopt, std::nullopt, includeSecureCookies, ApplyTrackingPrevention::No, ShouldRelaxThirdPartyCookieBlocking::No, IsKnownCrossSiteTracker::No).first;
			if (!cookieHeaderField.isEmpty())
				request.addHTTPHeaderField(HTTPHeaderName::Cookie, cookieHeaderField);
		}

		auto curlRequest = CurlRequest::create(WTF::move(request), *this);
		return curlRequest;
	}
			...
			{
				{
					...
					m_buffer.append(WTF::move(buffer));
...
public:
	AcinerellaNetworkFileRequestInternal(const String &url, Function<void(bool)>&& onFinished)
		: AcinerellaNetworkFileRequest(url, WTF::move(onFinished))
	{
		m_request = ResourceRequest(String(m_url));
		m_request.setCachePolicy(ResourceRequestCachePolicy::DoNotUseAnyCache);
		m_curlRequest = createCurlRequest(m_request);
...
{
	return adoptRef(*new AcinerellaNetworkFileRequestInternal(url, WTF::move(onFinished)));
}
