...
struct WebViewDelegate
{
	...
	std::function<void(const WebCore::ResourceResponse& response,
		const WebCore::ResourceRequest& request, const WTF::String& downloadAttribute, WebCore::FramePolicyFunction&& function)> _fDownloadAsk;
    ...
    std::function<void(void)> _fTextChanged;
